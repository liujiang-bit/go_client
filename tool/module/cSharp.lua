...
local function _write_read_field(field, stream, deep)
  ...
  elseif key then
    assert(is_array)
    local namesp = typename
    if namesp:find(".request") then
      namesp = namesp:gsub(".request.","/request/")
    end

    if namesp:find(".response") then
      namesp = namesp:gsub(".response.","/response/")
    end
    if namesp:find(".") then
      -- print(namesp,namesp:gsub("%.","/"))
      namesp = namesp:gsub("%.","/")
    end

    local fmt = string.format("this.%s = base.deserialize.read_map<%s, %s>(v => v.%s,\"SprotoType.%s\");", name, _typename2internal(key), typename, key.name, namesp)
    ...
    stream:write("this."..name.." = base.deserialize."..func_name.."<"..typename..">".." (\"SprotoType."..namesp.."\");", deep+1)
...
local function dump_class(class_info, stream, deep ,protocolClass,mainClass)
  ...
  if sproto_type then
    ...
    for i=1,#sproto_type do
	  ...
	  stream:write(sformat("private bool _has%s; // tag %d", upper_head(name), tag), deep)
      stream:write("public bool Has"..upper_head(name).." {", deep)
        stream:write(sformat("get { return base.has_field.has_field (%d); }", i-1), deep+1)
		stream:write(sformat("set { base.has_field.set_field (%d, true); _has%s = value; }", i-1, upper_head(name)), deep+1)
      stream:write("}\n", deep)
    end
    ...
    for k,v in pairs(protocolClass.classes) do
        if class_name ==  v.value.name then
          ...
          stream:write(string.format("public const string TagName = \"%s\";",v.value.tag), deep+1)
  ...
  deep = deep + 1
    for _,class_info in ipairs(class) do
      ...
      if request_type then
        ...
        stream:write("Protocol.SetRequest<"..request_type.."> ("..type_namespace.."."..stag..",\""..request_type:gsub(".request","/request").."\");",deep)
      end

      if response_type then
        response_type = type_namespace.."."..response_type
        stream:write("Protocol.SetResponse<"..response_type.."> ("..type_namespace.."."..stag..",\""..response_type:gsub(".response","/response").."\");",deep)
      end
      stream:write()
    end
  deep = deep - 1
