...
long loopread(BZFILE* d,int*bzerror,u_char *buf,size_t nbytes)
{
	long ptr,lenread;
...
int bspatch2(const char* old_file, const char* new_file, const char* patch_file)
{
    FILE* ctrlpipe,*diffpipe,*extrapipe=0;
    BZFILE* ctrlbz, *diffbz, *extrabz=0;
	...
	if(((fd=fopen(patch_file,"rb"))<=0) ||
		(fseek(fd,0,SEEK_END)!=0) ||
		((patchsize=ftell(fd)) == 0) ||
		(fseek(fd,0,SEEK_SET)!=0)) errx(1,"Problem patchsize %s", patch_file);
	...
	*/
	...
	if(fread(header,1,32,fd)!=32) errx(1,"Error reading %s", patch_file);
	...
	ctrlpipe=bz2read(&ctrlbz, &ctrlerror, 32,bzctrllen, patch_file);
	diffpipe=bz2read(&diffbz, &differror, 32+bzctrllen,bzdatalen, patch_file);
	if(version==2) {
		extrapipe=bz2read(&extrabz, &extraerror, 32+bzctrllen+bzdatalen,
			patchsize-(32+bzctrllen+bzdatalen), patch_file);
	};

	if(fclose(fd)==-1) errx(1,"Problem closing %s", patch_file);
	if(((fd=fopen(old_file,"rb"))<=0) ||
		(fseek(fd,0,SEEK_END)!=0) ||
		((oldsize=ftell(fd))==-1) ||
		((old=(u_char*)malloc(oldsize+1))==NULL) ||
		(fseek(fd,0,SEEK_SET)!=0) ||
		(fread(old,1,oldsize,fd)!=oldsize) ||
		(fclose(fd)==-1)) errx(1,"%s", old_file);
