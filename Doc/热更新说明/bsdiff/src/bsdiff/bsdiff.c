...
int bsdiff2(const char* old_file, const char* new_file, const char* patch_file)
{
	FILE* fd = NULL;
	BZFILE * bz = NULL;
	u_char *old=0,*pnew=0;
	...
	if(fwrite(header,1,32,fd)!=32) err(1,"%s", patch_file);
	...
	if((fseek(fd,0,SEEK_END))!=0 || ((len=ftell(fd))==-1)) err(1,"problem 0: %s",patch_file);
	offtout(len-32,buf);
	if((fseek(fd,8,SEEK_SET)!=0) || (ftell(fd) != 8) || (fwrite(buf,1,8,fd)!=8))
		err(1,"problem 1: %s",patch_file);
	offtout(newsize,buf);
	if((fseek(fd,24,SEEK_SET)!=0) || (ftell(fd) != 24)|| (fwrite(buf,1,8,fd)!=8))
		err(1,"problem 2:%s",patch_file);

	if(fseek(fd,0,SEEK_END)!=0) err(1,"problem 3: %s",patch_file);
	...
	if((fseek(fd,0,SEEK_END)!=0) || ((newsize=ftell(fd))==-1)) err(1,"problem 4: %s",patch_file);
	offtout(newsize-len,buf);
	if((fseek(fd,16,SEEK_SET)!=0) || (ftell(fd)!=16) || (fwrite(buf,1,8,fd)!=8))
		err(1,"problem 5:%s", patch_file);

	if(fseek(fd,0,SEEK_END)!=0 || (ftell(fd) == -1)) err(1,"problem 6: %s", patch_file);
