...
namespace ROK
{
    public class LodCamera : LodBase
    {
        ...
        public static LodCamera instance;
        ...
        private void Awake()
        {
            LodCamera.instance = this;
            OcclusionCullMgr.InitCullObj(base.transform.Find("cull_plane").gameObject);
        }

        public static void ReadLodArrayFromGameDataS(LodCamera self)
        {
        ...
        {
            int num = Mathf.RoundToInt(this.m_map_piece_width_curve.Evaluate(Common.GetLodDistance()));
            MapPieceManager.GetInstance().m_piece_width = (float)(num / 10 * 10);
            int num2 = Mathf.RoundToInt(this.m_map_piece_plane_width_curve.Evaluate(Common.GetLodDistance()));
            MapPieceManager.GetInstance().m_piece_plane_width = (float)(num2 / 10 * 10);
            MapDataManager.GetInstance().forceUpdateOnce = true;
            MapPieceManager.GetInstance().forceUpdateOnce = true;
        }

        public float GetMapPieceWidth()
        {
            return this.m_map_piece_width_curve.Evaluate(Common.GetLodDistance());
        }
