...
namespace ROK
{
    public static class LinePolygon
    {
        public static Mesh CreateMesh(Vector2[] points, float width, float uvStep = 0f)
        {
            ...
            Mesh mesh = new Mesh();
            mesh.vertices = LinePolygon.CreateLineVerticies(points, width, uvStep, ref uv);
            mesh.triangles = LinePolygon.CreateTriangles(points.Length);
            mesh.uv = uv;
        ...
        public static Vector3[] CreateLineVerticies(Vector2[] points, float width, float uvStep, ref Vector2[] uvs)
        {
            ...
            Vector3 vector;
            Vector3 vector2;
            LinePolygon.CalculateUpDownPoints(points[0], num3, num4, width, out vector, out vector2);
            array[0] = vector;
            array[1] = vector2;
            if (flag)
            {
                ...
                {
                    num2 = points[0].y - Mathf.Floor(points[0].y / uvStep) * uvStep;
                }
                else
                {
                    num2 = points[0].x - Mathf.Floor(points[0].x / uvStep) * uvStep;
                }
                float x = num2 / uvStep;
            ...
            for (int i = 1; i < points.Length - 1; i++)
            {
                ...
                num5 = Mathf.Sin(f2);
                num6 = Mathf.Cos(f2);
                LinePolygon.CalculateUpDownPoints(points[i], num5, num6, width, out vector3, out vector4);
                Vector3 vector5;
                Vector3 vector6;
                LinePolygon.CalculateMidiumPoint(vector, vector2, vector3, vector4, num3, num4, num5, num6, out vector5, out vector6);
                int num7 = i << 1;
                array[num7] = vector5;
                array[num7 + 1] = vector6;
                if (flag)
                {
            ...
            num5 = Mathf.Sin(f2);
            num6 = Mathf.Cos(f2);
            LinePolygon.CalculateUpDownPoints(points[num8 - 1], num5, num6, width, out vector3, out vector4);
            array[num - 2] = vector3;
            array[num - 1] = vector4;
            if (flag)
            {
        ...
        {
            float num = -sin_theta1 * cos_theta2 + sin_theta2 * cos_theta1;
            if (num == 0f)
            {
            ...
            float num3 = (vector.x * sin_theta1 - vector.z * cos_theta1) * -sin_theta2 + (vector3.x * sin_theta2 - vector3.z * cos_theta2) * sin_theta1;
            float num4 = (vector2.x * sin_theta1 - vector2.z * cos_theta1) * cos_theta2 - (vector4.x * sin_theta2 - vector4.z * cos_theta2) * cos_theta1;
            float num5 = (vector2.x * sin_theta1 - vector2.z * cos_theta1) * -sin_theta2 + (vector4.x * sin_theta2 - vector4.z * cos_theta2) * sin_theta1;
            p1 = new Vector3(num2 / -num, 0f, num3 / num);
            p2 = new Vector3(num4 / -num, 0f, num5 / num);
        }
    }
}